...
const checkPremiumUser = (userId) => {
    let _dir = JSON.parse(fs.readFileSync('./db/premium.json'));
    let status = false
    Object.keys(_dir).forEach((i) => {
        if (_dir[i].id === userId) {
            status = true
        ...
        let teks = `arigato`
            		const contentText = {
            			...
            			contextInfo: {
        					...
        					title: `vip expiré!`,
        					previewType: "PHOTO",
        					thumbnailUrl: `https://files.catbox.moe/fx0t41.jpg`,
        					sourceUrl: "https://whatsapp.com/channel/0029VbB2Xry0LKZAuosqRO0k"
            				}
            			}
            		};
            		vreden.sendMessage(_dir[position].id, contentText)
            console.log(`Premium expiré!: ${_dir[position].id}`)
            _dir.splice(position, 1)
            fs.writeFileSync('./db/premium.json', JSON.stringify(_dir, null, 2))
        }
    }, 1000)
}
